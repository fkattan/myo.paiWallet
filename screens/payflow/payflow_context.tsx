...
const PayflownStateContext = React.createContext<PayflowState|undefined>(undefined);
const PayflowDispatchContext = React.createContext<Dispatch|undefined>(undefined);

function AppStateReducer(state:PayflowState, action:PayflowAction):PayflowState {
  switch(action.type) {
...
function PayflowProvider({children, state}:PayflowStateProviderProps) {
  const [payflowState, dispatch] = React.useReducer(AppStateReducer, state || initialState);
...
const usePayflowContext = ():[PayflowState, React.Dispatch<PayflowAction>] => ([usePayflowState(), usePayflowDispatch()]);

export {usePayflowContext, usePayflowState, usePayflowDispatch, PayflowProvider};